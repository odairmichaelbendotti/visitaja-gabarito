...
- **Proteger uma rota**: envolva o retorno da `page.tsx` em `<RequireAuth>` (`lib/auth/RequireAuth.tsx`) — sem sessão, redireciona para `/login?redirecionar=<rota>`; a tela de login manda de volta pra lá depois de "entrar". Passe `requirePlan` para também exigir que `user.plan` esteja definido, redirecionando para `/onboarding` quando faltar. Ver `app/assinatura/escolher-plano/page.tsx` como exemplo.
- O `EXAMPLE_USER` (`lib/store/user.ts`) está com `plan: null` — simula alguém sem imobiliária/plano vinculado, então login/cadastro caem em `/onboarding` (→ `/assinatura/escolher-plano`). Para testar como cliente já assinante, mude esse campo para um id de plano (ex.: `"advanced"`) — login/cadastro passam a cair direto em `/visao-geral`.
- **Route group do dashboard**: as páginas internas do dashboard (`/visao-geral`, `/corretores`, `/imoveis`, `/configuracoes`) ficam em `app/(dashboard)/` — o parêntese é só organização de pastas e não aparece na URL. Nova página do dashboard (com `Sidebar` + `RequireAuth`) entra nessa pasta.
...
- **Fluxo de assinatura** (`app/assinatura/`): diferente do `(dashboard)`, essa pasta **não** tem parêntese — o nome `assinatura` faz parte da URL de propósito (`/assinatura/escolher-plano`, `/assinatura/imobiliaria`, `/assinatura/pagamento`, `/assinatura/pronto`). É o wizard de 4 passos (Plano → Imobiliária → Pagamento → Pronto) que aparece depois do login de quem ainda não tem plano. `layout.tsx` tem o cabeçalho compartilhado (logo + `StepIndicator.tsx`, que descobre o passo atual pela URL); `plans.ts` é a referência única do catálogo de planos (nome/preço/faixa) usada por todas as etapas.
...
O código (nomes de arquivo, componentes, props, variáveis, valores de variante, comentários) é em **inglês**. O que fica em **português**: texto visível na tela (rótulos, mensagens, placeholders), endereços de página (`/assinatura/escolher-plano`, `/criar-conta` etc.) e âncoras (`#como-funciona`), e dados de exemplo (nomes, e-mails fictícios). Ex.: `Botao` → `Button`, prop `variante` → `variant` com valores `"primario"` → `"primary"`; mas o texto do botão continua "Agendar visita".
